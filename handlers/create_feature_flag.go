...
func CreateFeatureFlag(db *sql.DB) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if err != nil {
			utils.JSON(c, http.StatusInternalServerError, gin.H{"message": "Failed to create feature flag", "input": input, "error": err.Error()})
			return
		}

		createdFlagId, err := res.LastInsertId()
		if err != nil {
			utils.JSON(c, http.StatusInternalServerError, gin.H{"message": "Failed to retrieve inserted ID"})
			return
		}
		...
		if err := row.Scan(&newFlag.ID, &newFlag.KEY, &newFlag.DESCRIPTION, &newFlag.ENABLED, &newFlag.CREATED_AT); err != nil {
			utils.JSON(c, http.StatusInternalServerError, gin.H{"message": "Failed to retrieve inserted flag", "ID": createdFlagId, "error": err.Error()})
			return
		}

		utils.JSON(c, http.StatusCreated, newFlag)
	}
}
